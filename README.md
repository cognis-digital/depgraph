...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3500&pause=1000&color=6B46C1&center=true&vCenter=true&width=720&lines=Dependency+risk+visualizer++Scorecard++OSV++typosquat++maint;Self-hostable+%C2%B7+MCP-native+%C2%B7+CI-ready+%C2%B7+polyglot" width="720"/>



[![PyPI](https://img.shields.io/pypi/v/cognis-depgraph.svg?color=6b46c1)](https://pypi.org/project/cognis-depgraph/) [![CI](https://github.com/cognis-digital/depgraph/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/depgraph/actions) [![License: COCL 1.0](https://img.shields.io/badge/License-COCL%201.0-2b6cb0.svg)](LICENSE) [![Suite](https://img.shields.io/badge/Cognis-Neural%20Suite-6b46c1.svg)](https://github.com/cognis-digital)
...
2. **Audit a dependency manifest** (`requirements.txt`, `package.json`, or `Pipfile`) and grade every package. Omit the path to read requirements from stdin:
   ```bash
   depgraph audit requirements.txt
   ```

3. **Browse the bundled advisory database**, optionally filtered by ecosystem:
   ```bash
   depgraph advisories --ecosystem pypi
   ```

4. **Read the output.** Emit JSON and gate on severity or the overall project grade:
   ```bash
   depgraph audit requirements.txt --format json --min-severity HIGH > deps.json
   ```

5. **Wire it into CI** — fail when the project grade drops to or below a letter:
   ```bash
...
- **MCP server** — `depgraph mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))

- **OpenAI-compatible / JSON** — pipe `depgraph scan . --format json` into any agent or LLM
...
Pipes into your stack: **SARIF** for code-scanning, **JSON** for anything, an **MCP server** (`depgraph mcp`) for AI agents, and a webhook forwarder for SIEM/Slack/Jira. See [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md).
...
| `scripts/setup-linux.sh` | `scripts/setup-macos.sh` | `scripts/setup-windows.ps1` | `docker run ghcr.io/cognis-digital/depgraph` | [DEPLOY.md](docs/DEPLOY.md) (AWS/Azure/GCP/k8s) |
