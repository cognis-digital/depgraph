"""Native cognis-connect emit for depgraph — forward findings to any platform.

Maps depgraph's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    depgraph ... --format json | depgraph-emit --to stix
    depgraph-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('name', '') + ' ' + rec.get('version', '')
        out['severity'] = rec.get('max_severity', 'info')
