...
TOOL_NAME = "DEPGRAPH"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'DG-CVE-001', 'severity': 'high', 'weight': 2.5, 'title': 'DEPENDENCY_HAS_CVE', 'when': {'open_cves': '^[1-9]'}, 'remediation': 'Upgrade or replace dependency.'}, {'id': 'DG-SCORE-001', 'severity': 'medium', 'weight': 2.0, 'title': 'LOW_SCORECARD', 'when': {'scorecard': '^[0-4]\\.\\d'}, 'remediation': "Maintain a fork or replace; the project's security posture is poor."}, {'id': 'DG-TYPO-001', 'severity': 'critical', 'weight': 3.0, 'title': 'TYPOSQUAT_RISK', 'when': {'typosquat_distance': '^[1-2]$'}, 'remediation': 'Verify correct package name; remove the typosquat dependency.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="sca",
                    metadata=item,
                ))
